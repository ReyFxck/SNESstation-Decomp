...
BUILD = ROOT / "build/matching/progress60-source-lineage"
...
def die(msg: str) -> None:
    raise SystemExit(f"Progress60 source-lineage: {msg}")
...
# ---------------------------------------------------------------------------
# Progress60: source-lineage recovery from Progress59 byte-level near misses.
#
...
def require_replace(text: str, old: str, new: str, label: str) -> str:
    if old not in text:
        die(f"Progress60 source-shape anchor missing ({label}): {old!r}")
...
print("=== Progress60 source-lineage recovery ===")
...
if "unsigned int length;    // 96" in p4:
    ...
    if second < 0:
        die("Progress60 could not locate NEW_PADMAN length field")
    p4 = p4[:second] + "int length;" + p4[second + len(needle):]
else:
    die("Progress60 could not locate pad_data length field")
...
for typename in ("unsigned int", "u32", "int"):
    s = str_text0
    s = require_replace(s, "#include <string.h>", "/* Progress60: string.h prototype hidden for 32-bit n probe */", "hide string.h")
...
zip_hdr_variant = VARIANT_DIR / "legacy_zip_recovered_p60.h"
...
fill_text = require_replace(
    ...
    '#include "legacy_zip_recovered_p60.h"',
...
if "unsigned g_legacy_zip_bits_left;" in fill_text:
    ...
    die("Progress60 could not establish signed bits_left source shape")
...
if anchor not in mc_text0:
    die("Progress60 could not locate historical mcRename")
head, tail = mc_text0.split(anchor, 1)
if call_set not in tail:
    die("Progress60 could not locate mcRename RPC selector")
...
if "char path[257]; /* target writes a terminator one byte beyond RPC payload */" in fio_text:
    fio_text = require_replace(
        ...
        "char path[256]; /* Progress60 exact-shape probe: target writes path[256] */",
        "fioMkdir local size",
    )
elif "char path[256];" not in fio_text:
    die("Progress60 could not establish fioMkdir 0x100-byte local frame")
...
lines = [
    "Progress60 source-lineage recovery",
