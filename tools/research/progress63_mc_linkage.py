...
BUILD = ROOT / "build/matching/progress63-mc-linkage"
...
def die(msg: str) -> None:
    raise SystemExit(f"Progress63 source-lineage: {msg}")
...
# ---------------------------------------------------------------------------
# Progress63: source-lineage recovery from Progress59 byte-level near misses.
#
...
def require_replace(text: str, old: str, new: str, label: str) -> str:
    if old not in text:
        die(f"Progress63 source-shape anchor missing ({label}): {old!r}")
...
print("=== Progress63 source-lineage recovery ===")
...
if "unsigned int length;    // 96" in p4:
    ...
    if second < 0:
        die("Progress63 could not locate NEW_PADMAN length field")
    p4 = p4[:second] + "int length;" + p4[second + len(needle):]
else:
    die("Progress63 could not locate pad_data length field")
pad_variants.append(("libpad-newpadman-8x2-loop-lenint", p4))

# Progress63: the target padInfoMode has `slt v0,v0,s1` at +0xb8,
...
for typename in ("unsigned int", "u32", "int"):
    s = str_text0
    s = require_replace(s, "#include <string.h>", "/* Progress63: string.h prototype hidden for 32-bit n probe */", "hide string.h")
...
if "unsigned g_legacy_zip_bits_left;" in fill_text:
    ...
    die("Progress63 could not establish signed bits_left source shape")
...
if anchor not in mc_text0:
    die("Progress63 could not locate historical mcRename")
head, tail = mc_text0.split(anchor, 1)
if call_set not in tail:
    die("Progress63 could not locate mcRename RPC selector")
...
# ---------------------------------------------------------------------------
# Progress63: mcFormat/mcUnformat have an identical 25-byte near miss.  Target
# disassembly keeps separate symbol bases for g_descParam, g_rdata and g_cdata
# (and therefore keeps g_descParam's base live in t4), while this historical
# snapshot declares all three static and GCC folds them into section-relative
# .bss references.  Probe linkage visibility directly instead of flag farming.
#
# The copy32 mcRename variant is used only as the already-established historical
# baseline for this TU; mcRename itself is already MATCHING and map_symbol()
# therefore excludes it from new-match accounting.
# ---------------------------------------------------------------------------
def globalize_decl(text: str, symbol: str) -> str:
    patterns = {
        "g_descParam": (
            r"static(\s+struct\s*\{[^}]*\}\s+g_descParam\s+"
            r"__attribute__\(\(aligned\(64\)\)\);)"
        ),
        "g_cdata": (
            r"static(\s+SifRpcClientData_t\s+g_cdata\s+"
            r"__attribute__\(\(aligned\(64\)\)\);)"
        ),
        "g_rdata": (
            r"static(\s+u8\s+g_rdata\[RSIZE\]\s+"
            r"__attribute__\(\(aligned\(64\)\)\);)"
        ),
        "mcRpcCmd": r"static(\s+const\s+int\s+mcRpcCmd\[2\]\[17\]\s*=)",
        "g_mclibInited": r"static(\s+int\s+g_mclibInited\s*=\s*0\s*;)",
        "g_currentCmd": r"static(\s+unsigned\s+int\s+g_currentCmd\s*=\s*0\s*;)",
        "g_mcType": r"static(\s+int\s+g_mcType\s*=\s*MC_TYPE_MC\s*;)",
    }
    pattern = patterns[symbol]
    out, count = re.subn(pattern, r"\1", text, count=1, flags=re.S)
    if count != 1:
        die(f"Progress63 could not globalize historical {symbol} declaration")
    return out

def globalize_many(text: str, symbols: tuple[str, ...]) -> str:
    for symbol in symbols:
        text = globalize_decl(text, symbol)
    return text

linkage_base = m1
linkage_specs = [
    ("libmc-desc-global", ("g_descParam",)),
    ("libmc-desc-cdata-global", ("g_descParam", "g_cdata")),
    ("libmc-desc-rdata-global", ("g_descParam", "g_rdata")),
    (
        "libmc-desc-cdata-rdata-global",
        ("g_descParam", "g_cdata", "g_rdata"),
    ),
    (
        "libmc-rpc-symbols-global",
        ("g_descParam", "g_cdata", "g_rdata", "mcRpcCmd"),
    ),
    (
        "libmc-core-symbols-global",
        (
            "g_descParam", "g_cdata", "g_rdata", "mcRpcCmd",
...
for name, symbols in linkage_specs:
    mc_variants.append((name, globalize_many(linkage_base, symbols)))
...
if "char path[257]; /* target writes a terminator one byte beyond RPC payload */" in fio_text:
    fio_text = require_replace(
        ...
        "char path[256]; /* Progress63 exact-shape probe: target writes path[256] */",
        "fioMkdir local size",
    )
elif "char path[256];" not in fio_text:
    die("Progress63 could not establish fioMkdir 0x100-byte local frame")
...
lines = [
    "Progress63 source-lineage recovery",
