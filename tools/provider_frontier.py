...
def render_runtime_c() -> str:
    # Old-GCC-compatible source; no target headers or modern libc are needed.
    return r'''/* Generated by tools/provider_frontier.py; ignored build product. */
#include <stdarg.h>
...
int snprintf(char *dst, p_size_t size, const char *format, ...)
{
    ...
    result = ps2lib_vsnprintf_recovered(dst, size, format, args);
    va_end(args);
    return result;
}
...
def link_frontier(args: argparse.Namespace, rows: Sequence[dict[str, str]]) -> dict[str, object]:
    ...
    if provider_defined != emitted or provider_undefined != {"ps2lib_vsnprintf_recovered"}:
        fail(
