#!/usr/bin/env python3
"""Rebuild and verify the strict V77 C4DrawWireFrame match.

The runner starts from the hash-pinned official Snes9x 1.41-1 source, applies
the target-proven SNES Station PS2 packed word and three-byte access forms,
compiles with the historical EE GCC 3.2.2 C++ frontend, and compares the
complete function against the private unpacked ELF. Only precise MIPS
relocation fields are normalized; no instruction byte is otherwise ignored.
"""
...
BUILD = ROOT / "build" / "matching" / "hunt1041-v77-c4draw"
...
ADDRESS = 0x0010CDCC
END_ADDRESS = 0x0010CFA4
EXPECTED_NAME = "snes_p17_0010cdcc"
HISTORICAL_IDENTITY = "C4DrawWireFrame"
AREA = "frontend-core"
OBJECT_SYMBOL = "_Z15C4DrawWireFramev"
PROVENANCE = "snes9x-1.41-1-official-plus-snesstation-ps2-packed-c4-access"
PROFILE = "snes9x-1.41-1-os-normal-double-ps2-packed-readword-read3-mask-first-fno-builtin"

PATCHED_SOURCE_SHA256 = (
    "e0c8151906f06ff51d433ac04427f5d826485f76782dfba3fd2acfc98da3c5fe"
)
PATCHED_MEMMAP_SHA256 = (
    "52df4fda21563b2ffadedc4cd7ecd50705b1476afb00e1ea384fc8f1ab9dab9d"
)
...
BYTEWISE_READ_3WORD = r'''#define READ_3WORD(s) ( *(uint8 *) (s) |\
                       (*((uint8 *) (s) + 1) << 8) |\
                       (*((uint8 *) (s) + 2) << 16))'''

PACKED_READ_3WORD = r'''#define READ_3WORD(s) ({ \
    register uint8 *source = (uint8 *) (s); \
    __asm__ volatile ("" : "+r" (source)); \
...
def make_evidence(target: bytes, built: v51.ObjectBuild) -> dict[str, str]:
    ...
    if (
        ...
        raise SystemExit("0x0010cdcc: manifest identity or boundary changed")
    elf = ELFFile(built.path)
    symbol = elf.find_symbol(OBJECT_SYMBOL)
    size = END_ADDRESS - ADDRESS
    if symbol.size != size:
        raise SystemExit(f"0x0010cdcc: object size changed from {size} to {symbol.size}")
    ...
    if actual_span_hash != TARGET_SPAN_SHA256:
        raise SystemExit(f"0x0010cdcc: target span SHA-256 mismatch: {actual_span_hash}")
    return {
        ...
        "detail": (
            "historical-symbol-strict; packed 32-bit unaligned READ_3WORD masked "
            "to 24 bits; pointer barrier preserves the EE unaligned pair; declaring "
            "the mask before the loaded value reproduces the target allocation"
        ),
...
def promote(evidence: dict[str, str]) -> tuple[int, int]:
    ...
    marker = "HUNT1041 V77 strict MATCH;"
    provisional_note = (
...
def main() -> None:
    ...
    print("V77 strict C4DrawWireFrame formal match: 1/1")
    print(f"target gate: formal-unpacked-elf:{TARGET_SHA256}")
    print("object_size=472; differing_bytes=0; unknown_relocations=none")
