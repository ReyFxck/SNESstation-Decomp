...
BUILD = ROOT / "build/matching/progress65-ps2lib-2004"
...
def die(msg: str) -> None:
    raise SystemExit(f"Progress65 ps2lib-2004: {msg}")
...
# ---------------------------------------------------------------------------
# Progress65: exact ps2lib-era libmc source probe.
#
# NON-MUTATING.
#
# The target is SNES Station v0.23 WIP dated 2004-01-24.  The PS2SDK history
# records that libmc was moved from ps2lib into ps2sdk on 2004-04-15 in commit
# 01d625018c3fde3044292446c910b8ea508adfdc.  That migrated source has the
# old mcCmd layout used by the target:
#
#   mcCmd.port @ +0
#   mcCmd.slot @ +4
#
# and mcFormat/mcUnformat intentionally call SifCallRpc with &mcCmd and a
# hard-coded send size of 48 bytes.  The target machine code exhibits exactly
# those offsets and the 0x30 send size.
#
# This probe compiles that historical source verbatim with the pinned EE GCC
# and evaluates every mapped RECONSTRUCTED function in the translation unit.
# No target bytes are copied into source and no progress status is mutated.
# ---------------------------------------------------------------------------
...
# Initialize the historical checkout with sparse paths before the first fetch,
# so this remains phone-friendly.
...
old_mc = PS2SDK_2004 / "ee/rpc/memorycard/src/libmc.c"
if not old_mc.is_file():
    die(f"historical libmc source missing after checkout: {old_mc}")

old_includes = sorted({
    p for p in PS2SDK_2004.rglob("include")
    if p.is_dir()
})
old_inc_flags: list[str] = []
for inc in old_includes:
    old_inc_flags += ["-I", str(inc)]

# Module include first, then exact 2004 include universe, then the already
# checked-out historical PS2DEV include set only as a last-resort compatibility
# fallback for headers not present in the migration snapshot.
mc_inc = PS2SDK_2004 / "ee/rpc/memorycard/include"
include_flags: list[str] = []
if mc_inc.is_dir():
    include_flags += ["-I", str(mc_inc)]
include_flags += old_inc_flags
...
provenance = "ps2lib-migration-20040415"

# -Os is the primary profile: all nearby historical source-lineage successes in
# this corridor use the size-optimized profile.  -O2 is retained as a cheap
# sanity cross-check, not a broad flag farm.
profiles = [
    ("p65-os", ["-Os"]),
    ("p65-o2", ["-O2"]),
]

print("=== Progress65 exact ps2lib-era libmc probe ===")
...
for profile_name, profile_flags in profiles:
    print(f"-- compile exact migrated ps2lib source [{profile_name}]", flush=True)
    obj = compile_source(
        old_mc,
        provenance=provenance,
    ...
    gained = evaluate_verbose(
        ...
        profile=profile_name,
        detail="exact-ps2lib-source",
    )

    rows = attempts[before_attempts:]
    for addr in (0x001A1554, 0x001A1610):
        row = next(
            (
                r for r in rows
                if int(str(r.get("address", "0x0")), 16) == addr
                and r.get("result") in ("MATCH", "mismatch")
            ),
            None,
        )
        if row is None:
            print(f"   0x{addr:08x}: n/a")
        elif row.get("result") == "MATCH":
            print(f"   0x{addr:08x}: MATCH")
        else:
            print(
                f"   0x{addr:08x}: diff={row.get('differing_bytes')} "
                f"first={row.get('first_differences')}"
            )
...
# Freeze the source identity only (the actual source remains in build/upstream).
source_sha = hashlib.sha256(old_mc.read_bytes()).hexdigest()
...
lines = [
    "Progress65 exact ps2lib-era libmc probe",
...
for addr in sorted(matches):
    ...
    lines.append(
        ...
        f"normalized_equal={r['normalized_equal']}"
    )
...
for r in best_rows[:40]:
    lines.append(
        ...
        f"{r.get('name')} profile={r.get('profile')} "
        f"first={r.get('first_differences')} "
...
if matches:
    ...
    print("No strict matches; inspect best.tsv for the exact pre-2004 delta.")
