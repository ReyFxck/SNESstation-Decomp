...
class ProjectStatusTests(unittest.TestCase):
    def test_checkpoint_accounting_is_disjoint_and_complete(self) -> None:
        ...
        self.assertEqual(status.formal_matching, 994)
        self.assertEqual(status.reconstructed_unproven, 47)
        self.assertEqual(status.recovered_pending, 0)
        self.assertEqual(status.working_checkpoint, 994)
        self.assertEqual(status.working_remaining, 47)
        self.assertEqual(
