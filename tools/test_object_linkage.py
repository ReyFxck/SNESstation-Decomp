...
class ObjectLinkageAuditTests(unittest.TestCase):
    def test_frozen_manifest_matches_current_transport(self) -> None:
        ...
        self.assertEqual(document["result"]["direct_object_bytes"], 562_636)
        self.assertEqual(document["result"]["remaining_object_native_bytes"], 157_984)
    ...
    def test_candidate_object_inventory_is_explicit(self) -> None:
        ...
        self.assertEqual(result["direct_candidate_object_inputs"], 37)
        self.assertTrue(result["transport"]["selected_candidate_objects_linked_directly"])
        self.assertEqual(result["direct_object_inputs"], 42)
        self.assertEqual(result["direct_object_sections"], 184)
        self.assertEqual(result["incbin_payload_bytes"], 157_984)
