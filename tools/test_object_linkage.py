...
class ObjectLinkageAuditTests(unittest.TestCase):
    def test_frozen_manifest_matches_current_transport(self) -> None:
        ...
        self.assertEqual(document["result"]["direct_object_bytes"], 296_116)
        self.assertEqual(document["result"]["remaining_object_native_bytes"], 424_504)
    ...
    def test_candidate_object_inventory_is_explicit(self) -> None:
        ...
        self.assertEqual(result["direct_candidate_object_inputs"], 11)
        self.assertTrue(result["transport"]["selected_candidate_objects_linked_directly"])
        self.assertEqual(result["direct_object_inputs"], 16)
        self.assertEqual(result["direct_object_sections"], 147)
        self.assertEqual(result["incbin_payload_bytes"], 424_504)
        self.assertEqual(result["incbin_payload_sections"], 113)
