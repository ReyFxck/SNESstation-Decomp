...
- **Zero-byte link contracts:** **1,336/1,573 resolved**, **237 blocked** (1,273 address anchors + 63 semantic aliases)
- **Private embedded assets:** **10/10 providers**, **62,736 verified private bytes**, **227 remaining externals**
- **Source-link provider namespace:** **227/227 resolved**, **0 aggregate externals** (175 anchors + 9 aliases + 39 storage + 4 shims)
...
aliases resolve 1,336/1,573 live contracts and reduce the provider frontier to
237, again without changing an allocated section. V90 explains the 20
target-absent Stage-3E contracts removed from source and the canonical `errno`
alias; the historical pre-refactor checkpoint is preserved in
...
The live aggregate falls from 237 to 227 unresolved providers while every
...
`make provider-frontier` consumes the private-asset aggregate and closes all 227
...
definitions and four deterministic EE runtime shims. The resulting relocatable
...
to zero and the partial link proves **227 → 0** externals. Four historical
runtime shims remain for Stage 3D. See
