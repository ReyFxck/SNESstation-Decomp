...
- **Zero-byte link contracts:** **1,336/1,570 resolved**, **234 blocked** (1,273 address anchors + 63 semantic aliases)
- **Private embedded assets:** **10/10 providers**, **62,736 verified private bytes**, **224 remaining externals**
- **Source-link provider namespace:** **224/224 resolved**, **0 aggregate externals** (175 anchors + 9 aliases + 39 storage + 1 shims)
...
- **Stage-3D libgcc contracts:** **7/7 closed** (**4 exact archive members + 3 completed source refactors**; runtime shims **4 → 1**)
...
aliases resolve 1,336/1,570 live contracts and reduce the provider frontier to
234, again without changing an allocated section. V90 explains the 20
target-absent Stage-3E contracts removed from source and the canonical `errno`
alias; V91 removes three compiler-generated lift artifacts. The historical
...
The live aggregate falls from 234 to 224 unresolved providers while every
...
`make provider-frontier` consumes the private-asset aggregate and closes all 224
...
definitions and one deterministic EE runtime shim. The resulting relocatable
aggregate has **zero undefined globals**. Compatibility storage/shims are
...
to zero and the partial link proves **224 → 0** externals. One historical
runtime shim remains for Stage 3D. See
...
source-only libcalls are removed from the compiled aggregate. Runtime shims
fall from four to one (`snprintf`). See
