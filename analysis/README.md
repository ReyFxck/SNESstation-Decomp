...
| `link_identity/` | Stage-3 unpacked-layout oracle, aliases, link-contract/provider closure and hash-only private-provider evidence |
...
- `link_identity/` — hash-only unpacked layout, zero-byte alias/anchor decisions, private-provider ranges and the complete source-link provider closure.
