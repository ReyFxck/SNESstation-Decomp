...
| `link_identity/` | Stage-3 unpacked-layout oracle, source-address aliases, link-contract frontier and hash-only private-provider evidence |
...
- `link_identity/` — hash-only unpacked layout, zero-byte alias/anchor decisions, private-provider ranges and the explicit whole-program frontier.
