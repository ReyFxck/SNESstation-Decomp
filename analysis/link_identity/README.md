...
- `link_contracts.tsv` classifies every one of the 1,570 live post-refactor
  externals: 1,273 absolute target-address anchors, 63 semantic text aliases
  and 234 explicit provider blockers.
...
- `provider_frontier_closure.tsv` classifies all 224 live names left after the
  private providers into 175 target-address anchors, nine recovered-text
  aliases, 39 compatibility storage definitions and one historical-runtime
  shim.
...
bytes and reduces the partial-link frontier from 234 to 224. No generated
...
links them after the private providers and proves **224 -> 0** undefined
...
compatibility storage reaches **39 -> 0** and the aggregate reaches **224 ->
0** undefined globals. Generated bytes remain below ignored
`build/named-contracts/`; final placement remains Stage 3G, and one runtime
shim remains for Stage 3D.
...
from the live aggregate, and leaves `snprintf` as the sole runtime shim.
