...
replacement ELF. Twenty-nine ELF inputs provide **491,796 code bytes** directly
through 182 placed code sections. Twenty-four are historical candidate objects,
including `2XSAI.o`, `fxemu.o`, `fxinst.o`, `SA1CPU.o`, `ppu-short.o`,
`CPUEXEC.o`, `dma.o`, `gfx-short.o`, `CPUOPS.o`, `tile.o` and eight original runtime objects.
...
so the proved semantic data provider remains. The other 73 candidate objects
...
The remaining **228,824 code bytes** pass through generated `.incbin` sections
...
| Bytes linked directly from producer objects | **491,796/720,620 (68.25%)** |
| Direct ELF object inputs | **29** (including **24** historical candidate objects) |
| Direct placed sections | **182** |
| Bytes remaining behind generated payload | **228,824** |
