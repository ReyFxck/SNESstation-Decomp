...
| Source-address alias tranche | **333/347 proved** | Zero-byte linker aliases bind proven alternate names to canonical global text symbols; 14 remain blocked. |
| Zero-byte link-contract frontier | **1,297/1,530 resolved** | 1,234 absolute target-address anchors plus 63 semantic text aliases leave 233 provider contracts explicit. |
...
| Stage-3F unnamed data accesses | **872/1265 with proved consumed spans** | 167,659 unique bytes, 10 fixed-count memory-call ranges; 146 fixed-point CFG and 33 deterministic-prefix witnesses. 354 lack access witnesses; 29 ROM-offset refactors are separately closed. These are not complete object/array bounds. |
...
cumulative proof to 333 of 347 aliases against
317 canonical global text symbols and applies them without changing
...
alias-resolved contract map has 1,530 rows. It still resolves
1,297 contracts without allocating a byte: 1,234
...
0 addresses, complete object bounds and final executable
...
the final V101 ledger is 1,265/1,265 with zero unresolved.
V102 starts Stage 3G with a clean ET_REL-to-ET_EXEC diagnostic: it fixes all
179 proved provider sections, preserves exact bytes in all
155 initialized fixed sections and matches
12/51 oracle windows. Its
...
V108 rebuilds 4 source sections / 31,460
bytes, applies 2,083 source relocations and emits
27 semantic FDEs. DSP1, all four CPU opcode tables,
`fxemu`, `fxinst` and renderer unwind metadata close window 36 exactly; the
cumulative diagnostic reaches 18/51
exact windows with 1,845,637 differing bytes. See
...
user-supplied reference. Their 1,284,388 asset bytes and
6 checked size words close all 20
...
4. **Address-alias tranche frozen:** 333/347 are proved and zero-byte bound; the remaining 14 are carried into the V85 provider frontier.
5. **Zero-byte link-contract tranche frozen:** 1,297/1,530 are resolved and the exact 233-name input provider frontier is classified.
