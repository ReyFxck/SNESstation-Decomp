...
| Reproducible EE compiler stage one | binutils `2.14` + GCC `3.2.2` C-only, PS2DEV commit `16a4718` patches | Both GNU archives and every recipe file are SHA-256 pinned; x86_64 builds pass; a native AArch64 artifact passes the R5900/ELF32 probe and the `mathfp` comparison on DroidSpaces | Supplies a compile-only comparison candidate. Exact `-b1` patch identity, Newlib/C++, SDK archives and final linking remain separate gates. |
